...
class UserService:
    ...
    async def submit_lead(self, name: str, contact: str, message: str) -> None:
        # Leads logic for legacy form (Web)
        await self.user_repo.save_lead(0, f"{name} | {contact}", message)

    async def save_bot_lead(self, user_id: int, contact: str, message: str) -> None:
        """Save lead from bot with user_id"""
        await self.user_repo.save_lead(user_id, contact, message)
